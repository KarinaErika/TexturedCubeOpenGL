...
int main(void)
{
	...
	//Carrega a textura usando dois métodos criados
	GLuint Texture = loadBMP_custom("C:/ogl-master/tutorial02_red_triangle/texture/container.bmp");
	//GLuint Texture = loadDDS("C:/ogl-master/tutorial02_red_triangle/texture//uvtemplate.DDS");
	...
	do {
		...
		//Verifica qual projeção o cubo aparecerá de acordo com a tecla que o usuário clicar
		processInput(mvp);
...
void processInput(mat4 &mvp){
	...
	if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS) {
		oblique = glm::mat4(1.0f);

		oblique[0][2] = -1 * (glm::cos(3.14 / 4));

		Projection = oblique * ortho(-3.0f, 3.0f, -3.0f, 3.0f, -0.1f, 100.0f);
	...
	if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS) {
		...
		Projection = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, 0.0f, 100.0f); //em coordenadas mundiais
		mvp = Projection * View * Model;
	}
